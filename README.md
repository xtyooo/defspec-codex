# specpilot-codex（规格领航 · Codex Skills 版）

> 🧭 让 Codex 在写代码前先读规格、建需求、确认边界、出方案，再进入实现。

`specpilot-codex` 是一套面向 Codex 的 SpecPilot 工作流安装器。它把 SpecPilot 安装为当前项目的 `.codex/skills/`，并初始化 `docs/specpilot/` 需求文档体系。

当前版本采用 **skills-only** 模式：不再依赖 Codex plugin、marketplace 或 `commands/`，入口更轻，行为也更接近 superpowers 的项目级 skills 安装方式。
...
你：给系统加一个导出功能
Codex：好的，我直接开始改代码...
结果：格式、权限、数据量、失败处理都在实现过程中才暴露
```

装了 SpecPilot：

```text
你：/specpilot:new 增加导出功能
Codex：先创建 REQ 草稿，记录需求、边界和上下文

你：/specpilot:confirm REQ-001
Codex：确认业务规则、数据流、异常场景和测试用例

你：/specpilot:exec REQ-001
Codex：先出方案，确认后再实施、测试、记录结果
```

需求、方案、测试和复盘都会沉淀在项目里，而不是只留在一次聊天上下文中。
...
| 能力 | 说明 |
|------|------|
| 🧩 Project Skills | 安装 9 个 `specpilot-*` skills 到 `.codex/skills/` |
| 📝 需求文档体系 | 初始化 `docs/specpilot/requirements/`、`specs/` 和项目指南 |
| 🗺️ 项目级上下文 | 通过 `project.md` 与 `project-guide.md` 记录项目差异 |
| 🔁 可重复安装 | 重复运行会更新 skills，默认保留已有 `docs/specpilot/` |
...
你应该能看到 9 个 `specpilot-*` skills 都是 `OK`。
...
| `specpilot-check` | 复核需求、方案、实现、测试和风险 |
| `specpilot-show` | 查看需求或能力规格详情 |
| `specpilot-update` | 处理需求变更 |
| `specpilot-archive` | 归档已完成需求 |
| `specpilot-cancel` | 取消需求但保留记录 |

这些 skills 的描述包含 `/specpilot:new`、`/specpilot:exec REQ-xxx` 等触发语，因此可以直接输入类似命令文本来触发对应流程。
...
默认不会写入 `AGENTS.md`。如果你希望项目显式保留 SpecPilot bootstrap：
...
## 🗺️ 初始化 project-guide.md

`docs/specpilot/project-guide.md` 是 SpecPilot 的项目地图。后续使用 `specpilot-new`、`specpilot-confirm`、`specpilot-exec`、`specpilot-check` 时，Codex 会先读取它来理解项目结构和开发约束。
...
- 默认不写入 `AGENTS.md`；需要时可使用 `--with-agents`。
- 安装时只清理名称为 `specpilot-*` 或 `defspec-*` 且包含 `SKILL.md` 的旧技能目录。
- 旧版 SpecPilot/DefSpec plugin 和 marketplace 配置会被清理，但不会创建新的 plugin 或 marketplace。
